...
- **No cloud LLMs**, with one scoped exception: **HolyClaude** interactive use (Claude Max/Pro subscription via native `claude login` OAuth, not Omniroute — reverted 2026-09-26, see the holyclaude bullet below) and **OpenCode** inside HolyClaude. It never extends to cron/webhook/automated workloads (argus included). Do not use it as precedent to relitigate the no-cloud policy or the OpenCode rejection. See `docs/ai-history/rejected-decisions.md`.
- **Anthropic (Claude) lane**: `cliproxyapi/claude-*` models, bare or via a combo, are interactive-only. As of 2026-09-26 there is no live consumer (HolyClaude went back to native OAuth) — the lane rule still stands for if/when one returns. Automated consumers use the OpenAI/Codex lane (`cliproxyapi/gpt-*`) or local models. Enforcement has gaps (Accepted risks); never add a Claude leg to a combo that an automated key uses.
...
- **holyclaude** (`holyclaude/`): interactive dev workstation, `holyclaude.68cc.io` (forwardAuth, external gateway). Image `docker.io/coderluii/holyclaude` `full` 1.6.3; OpenCode needs `full`.
  - **Reverted 2026-09-26**: Claude Code auth is native `claude login` OAuth again, talking directly to Anthropic — no `ANTHROPIC_BASE_URL`/API-key override, no Omniroute involvement. From 2026-09-16 to 2026-09-26 it went through Omniroute's `holyclaude-interactive` key on the `coding-deep` combo; that combo's fallback legs never worked (`gpt-5.5` rejected on request shape, the local model overflowed context) so it failed closed on any Omniroute hiccup instead of degrading, which is why it was reverted. See `docs/superpowers/plans/2026-09-26-holyclaude-omniroute-revert.md` and `docs/ai-history/holyclaude.md`. The `holyclaude-interactive` Omniroute key is no longer used by anything; it lives in Omniroute's DB, not git, so revoke it there if you want it gone.
  - `postStart` now only **cleans up** `~/.claude/settings.json` (strips any Omniroute-routing keys a pod booting from an already-patched PVC state would otherwise still carry) — it no longer writes `ANTHROPIC_*`/`model` into it. If the pre-existing OAuth session on the PVC has expired, run `claude login` once from the web terminal; it persists there the same way it did before 2026-09-16.
