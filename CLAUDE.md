...
- **llama-swap** — local GGUF inference, Vulkan via `ghcr.io/mostlygeek/llama-swap:vXXX-vulkan-bXXXX`. Pinned to `bigboi-jms-01` (Navi 48 dGPU = Radeon RX 9070 XT, RDNA4/gfx1201, device-id `0x7550`, 16 GiB VRAM, node-label `node.kubernetes.io/gpu-tier=dgpu`) via nodeAffinity. RDNA4 has a working Vulkan flash-attention path (b9803+) — `--flash-attn on` is beneficial here, NOT the RDNA2 no-coopmat case. The entry point for large/dense chat, reasoning, and coder models — `llm.68cc.io` route (Authentik forwardAuth) plus direct cluster-internal Service access. A second, narrower instance (`llama-swap-apu`, see below) now handles routing/HA-voice on `bee-jms-03`, so this is no longer literally the only entry point, just the source of truth for the models listed here. Hot-swap via `chat` group (exclusive); embed + rerank stay resident in `always-on` group. Init container pre-fetches GGUFs into the PVC. Model aliases (see `kubernetes/apps/ai/llama-swap/app/configmap.yaml` for source of truth for these; `fast`/`small`/`router` moved to `kubernetes/apps/ai/llama-swap-apu/app/configmap.yaml`, which is now its source of truth):
...
- **llama-swap-apu** — second llama-swap instance, hard-pinned to `bee-jms-03` (Renoir iGPU, BIOS-expanded to 16 GiB dedicated VRAM — see `docs/superpowers/specs/2026-08-16-apu-light-inference-tier-design.md` for the bee-* hardware asymmetry). Hosts only `qwen3-1.7b` (`fast`/`small`/`router`), always-on/non-exclusive. `coder-fim` (FIM autocomplete) was removed 2026-08-17 — its only consumer was Continue.dev, which is explicitly rejected (see "Decisions explicitly rejected" below). Cluster-internal only (`llama-swap-apu.ai.svc.cluster.local:8080`), no external route. `bee-jms-01`/`-02` are excluded (3 GiB stock dedicated VRAM, not worth targeting). No routing layer in front of the two llama-swap instances — consumers pick the endpoint that matches their model directly.
